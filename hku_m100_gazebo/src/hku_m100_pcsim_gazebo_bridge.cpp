...
void attitudeQuaternionCallback(const geometry_msgs::QuaternionStamped::ConstPtr& msg)
{
  
  target_pose.orientation = msg->quaternion;
...
{
  target_twist.linear = msg->vector;

  velocity_updated = true;
  // std::cout << "velocity callback get called" << std::endl;

  // const tf::Quaternion q_dji_flu(-target_pose.orientation.x,-target_pose.orientation.y,-target_pose.orientation.z,target_pose.orientation.w);

  // // tf::Vector3 = velocity.vector;
  // // tf::Matrix3x3 rot(q_dji_flu);

  // //rotating velocity vector to body frame
  // tfScalar x = target_twist.linear.x; 
  // tfScalar y = target_twist.linear.y; 
  // tfScalar z = target_twist.linear.z; 

  // tf::Vector3 vector(x,y,z);

  // tf::Vector3 rot_vector = tf::quatRotate(q_dji_flu,vector);

  // target_twist.angular.x = rot_vector.getX();
  // target_twist.angular.y = rot_vector.getY();
  // target_twist.angular.z = rot_vector.getZ();
